...
socket.on("Member", function(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        $("#contacts_list").append('<li class="contact"><div class="wrap"><span class="contact-status ' + status + '"></span>' + img + '<div class="meta">' +
...
socket.on("NewMessage", function(data) {
    console.log(data);
    $("#messages_list").append('<li class="sent">' + img +
        '<p>' + data.message + '</p>' +
        '</li>'
    );
...
function newMessage() {
    ...
    $('<li class="replies">' + img + '<p>' + message + '</p></li>').appendTo($('.messages ul'));
